...
def opt1():
    ...
    for file_name in os.listdir(current_path):
        ...
        # 默认输入 inp 文件，进行格式转换        
        if os.path.isfile(file_path) and (file_name=="atom.inp"):
            input_file_name = "atom.inp"
            input_file_path = file_path
            mark_inp_exits = True
            break
    
    # 若不存在 atom.inp 文件，则需要手动指明inp格式文件的文件名
    if mark_inp_exits == False:
        os.system('''        echo -e "\n\033[31m - 未搜索到名为atom.inp的结构文件，需要手动指定inp格式的文件名...\033[0m\n"''')
        input_file_name = input(" inp格式的文件名\n------------>>\n")
    ...
    ### Step 2. 文件格式转换
    structure = DStructure.from_file(file_path=input_file_path, file_format="inp")
