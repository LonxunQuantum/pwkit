...
def opt3():
    ...
    ---------
        ...
        2. output_file_path
    '''    
    mark_pwscf_exits = False
    
    ### Step 1. 得到输入输出格式的文件
    current_path = os.getcwd()
    for file_name in os.listdir(current_path):
        file_path = os.path.join(current_path, file_name)
        # 默认输入 atom.pwscf 文件，进行格式转换        
        if os.path.isfile(file_path) and (file_name=="atom.pwscf"):
            input_file_name = "atom.pwscf"
            input_file_path = file_path
            mark_pwscf_exits = True
            break
    
    # 若不存在 pwscf 文件，则需要手动指明pwscf格式文件的文件名
    if mark_pwscf_exits == False:
